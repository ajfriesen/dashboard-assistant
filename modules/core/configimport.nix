...
#      USB filesystem read-only and, if it holds ha-dashboard.yaml, imports it.
#   2. ESP on first boot: /boot is FAT and editable on any computer, so dropping
#      ha-dashboard.yaml there provisions a freshly flashed image on first boot.
#
...
let
  ...
  usbImport = pkgs.writeShellScript "ha-dashboard-usb-import" ''
    set -eu
    dev="$1"
    ...
    [ -f "$mnt/ha-dashboard.yaml" ] || exit 0
    echo "importing dashboard config from $dev"
    ${postToDaemon "\"$mnt/ha-dashboard.yaml\""}
  '';

  bootImport = pkgs.writeShellScript "ha-dashboard-boot-import" ''
    set -eu
    echo "importing dashboard config from /boot/ha-dashboard.yaml"
    ${postToDaemon "/boot/ha-dashboard.yaml"}
  '';
in
{
  options.dashboard.configImport.enable = lib.mkEnableOption ''
    importing dashboard config (HA URL, token, Wi-Fi) from a ha-dashboard.yaml
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.udev.extraRules = ''
      ACTION=="add", SUBSYSTEM=="block", ENV{ID_BUS}=="usb", ENV{DEVTYPE}=="partition", ENV{ID_FS_USAGE}=="filesystem", TAG+="systemd", ENV{SYSTEMD_WANTS}+="ha-dashboard-usb-import@%k.service"
    '';

    systemd.services."ha-dashboard-usb-import@" = {
    ...
    systemd.services.ha-dashboard-boot-import = {
      description = "Import dashboard config from /boot on first boot";
      wantedBy = [ "multi-user.target" ];
      after = [ "ha-dashboard-daemon.service" ];
      unitConfig.ConditionPathExists = [
        "/boot/ha-dashboard.yaml"
        "!/var/lib/dashboard/provisioned"
      ];
      serviceConfig = {
        Type = "oneshot";
